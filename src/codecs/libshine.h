...
/* Encode interleaved audio data into an mp3 frame. Source data must have
 * `shine_samples_per_pass(s)` audio samples per channels. Mono encoder only
 * expect one channel. 
 *
 * Returns number of bytes written to output buffer. */
int shine_encode_buffer_interleaved(shine_t s, int16_t *data, unsigned char *buf, int bufsize);

/* Close an encoder, freeing all associated memory. Encoder handler is not
 * valid after this call. */
void shine_close(shine_t s);

int shine_add_samples_interleaved(shine_t s, int16_t *data, int num_samples, unsigned char *buf, int bufsize);

int shine_flush_samples(shine_t s, unsigned char *buf, int bufsize);
